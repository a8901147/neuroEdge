...
**Grasp status**: `tools/mujoco_bridge/test_grasp_object.py`/`test_myoware_grip_replay.py` verify a real contact-based pickup (reach → close → lift, checking whether the object followed the hand) at the scene's current front-left reach pose, at `GRIP_SCALE=0.6` — both a hand-authored grip ramp and a real EMG-test-data-decoded grip hold there. `test_grasp_coverage.py`'s wider sweep (5 reach directions × 5 grip scales) shows this does **not** generalize: front-left is the only pose that ever holds, and even there it's non-monotonic across grip scale (holds at 0.30/0.60, drops at 0.45/0.80/1.00 — confirmed deterministic, not simulation noise); the other 4 tested poses fail at every grip scale, either dropping the object outright or never actually catching it during the grip phase. The uniform-curl grip (all 6 finger actuators scaled by the same fraction, not per-finger targets shaped to the object) is the suspected reason — see `test_grasp_coverage.py`'s own docstring for the full breakdown. `test_grip_kinematics.py` checks the hand's open/close range in isolation with no object.
