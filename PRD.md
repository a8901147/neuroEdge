...
* **Phase 1.5: 目標硬體可行性驗證 (Feasibility Spike) 🔥【當前主力目標】**
  ...
  * **階段 6：真實硬體即時串流驅動 MuJoCo，第二顆 MPU6050 + USART2 升速，程式碼已完成、實體驗證待接線（2026-08-22）**——目的修正：Phase 2 的 MuJoCo 橋接（iteration 1/2）目前都是 CSV 重播的原型，只是用來驗證 3D 模型/控制映射本身正確，**不是最終目的**。使用者提醒後重新確認：真正的目標是真實 STM32 + 真實 MyoWare + 真實 2 顆 MPU6050 即時解碼、即時驅動 MuJoCo，證明的是一個真的能動的嵌入式感測融合系統，不是重播錄好的資料。
    - **雙 IMU 共用同一條 I2C1 bus，靠位址區分**：上臂維持 `0x68`(`AD0` 不接)，前臂新接一顆、`AD0` 接 3.3V 得到 `0x69`。刻意不另外開一條 I2C2/I2C3——I2C 本來就是為多裝置定址設計的，I2C2 可用接腳跟除錯用的 JTDO 衝突（階段 4b 已經試過放棄）。`ImuReader` 原本把位址寫死成全域常數 `kImuTargetAddr`，改成建構子參數（`addr7_` member），`mpu6050_write_reg_blocking()` 也加一個位址參數；`main()` 建立 `shoulder_reader(0x68u)`/`elbow_reader(0x69u)` 兩個實例，用一個簡單的交替旗標（`active_is_shoulder`）確保同一時間只有一顆在進行 I2C 傳輸——**只在成功完成一次完整讀取時才切換到另一顆，逾時就留在同一顆重試**，跟原本單顆時的重試語意一致。
    - **預期吞吐量減半，這是設計取捨不是回歸**：階段 5c 量到單顆 591-593 次/秒是整條 bus 被一顆裝置獨佔的結果；兩顆交替共用同一條實體 bus，每次傳輸佔用的匯流排時間不會因為換位址而變短，預期每顆各自降到 ~290-295 次/秒左右。新增 `shoulder_completions`/`elbow_completions` 兩個計數器，之後接上真實硬體要實測確認，不是只看推算就相信。
    - **USART2 從 9600 升速到 115200，BRR 查證過，不是猜的**：新的輸出格式（`tick=<n> grip=<f> gripping=<0|1> shoulder_pitch=<f> shoulder_roll=<f> elbow=<f>`，跟 `tools/mujoco_bridge/run_demo.py` 既有的 `LINE_RE` 正規表達式完全對齊，讓 Python 端可以直接重用同一套解析邏輯）每行約 95 bytes，9600 鮑率（~960 bytes/秒）只夠撐每秒約 10 行，對即時手臂追蹤來說太卡。用 grep 確認整個 codebase 沒有任何檔案設定過 `RCC->CFGR`/`RCC->PLLCFGR`——目前所有階段都跑在未設定的重置時脈 HSI 16MHz，原本 9600 鮑率的 `BRR=0x0683` 也是對這個時脈算的。依 RM0368 19.3.4 公式（`Baud=fCK/(16×USARTDIV)`，`OVER8=0`）：`USARTDIV=16,000,000/(16×115200)=8.6875`，`Mantissa=8`、`Fraction=round(0.6875×16)=11`，`BRR=(8<<4)|11=0x8B`，實際鮑率 ≈115,108（誤差 0.08%，在容許範圍內）。輸出頻率也從原本每 1000 tick 一次（診斷用，1Hz）改成每 10 tick 一次（100Hz，~950 bytes/秒，鮑率預算的 8%），MuJoCo 自己的 `launch_passive` 迴圈本來就會依 `model.opt.timestep` 自行踩步調，100Hz 綽綽有餘。新增 `usart2_send_float()`：真正印出帶小數點的十進位數字（例如 `-0.123456`），不是舊的 `usart2_send_float_x1000` 那種「乘 1000 印成整數」——沿用這個檔案既有的手刻數字字串風格，沒有 `<cstdio>`/`printf`。
    - **姿態融合/肘角公式，直接從已測試的 CSV 原型搬過來**：`src/mujoco_bridge_demo.cpp` 已經驗證過這套邏輯（見上方 Phase 2 章節），韌體端幾乎逐字搬過來——兩個獨立 `ComplementaryFilter` 實例（`shoulder_filter`/`elbow_filter`，本身無共用/全域狀態，安全），各自從自己第一筆完成的加速度計讀值 `initialize()`（跳過冷啟動收斂暫態），`elbow = max(0, elbow_filter.pitch() - shoulder_filter.pitch())`。移除了階段 5b 原本接在後面的 `IirFilter` 平滑（`kSmoothAlpha=0.5`）——那組參數本來就標記為「還沒用真實資料驗證過」，拿掉之後韌體端輸出的就是 `filter.roll()/pitch()` 原始值，跟已測試的 Host 端原型完全一致，避免多一層沒驗證過的平滑造成兩邊行為分歧。
    - **Python 端新增 `tools/mujoco_bridge/run_demo_live.py`**：獨立腳本，不是在既有 `run_demo.py` 上加參數——即時序列埠沒有「播放完畢」這種明確結束訊號，跟 CSV 重播的生命週期本質不同，分開寫可以保護已經跑通、有 commit 記錄的 CSV 版本不被連帶改壞。沿用 `run_demo.py` 完全相同的 `LatestSample`、`LINE_RE`、MuJoCo actuator 映射、`launch_passive` 迴圈，只換掉資料來源（`pyserial` 讀 USART2，不是 `subprocess` 讀 CSV binary 的 stdout）。預設 115200 鮑率，跟這次韌體改動配對；**注意跟 `tools/watch_myoware_uart.py` 預設的 9600 不一樣**，兩支工具對應不同韌體階段，不要弄混。`pyserial` 已加進 `tools/mujoco_bridge/requirements.txt`。同時修正 `tools/check_hardware_ready.py` 裡硬式編碼的 9600（原本檢查序列埠是否能開啟時用的鮑率），改成 115200 對齊。
    - **驗證方式，SWD/暫存器優先，最後才排視覺/即時檢查（本專案一貫作法，不要求使用者用肉眼判斷燈號）**：程式碼已完成、編譯乾淨（`text=4104 data=536 bss=316`，遠低於 SRAM/Flash 上限），但**尚未接上實體硬體驗證**——待第二顆 MPU6050 實際接線（`AD0→3.3V`）後，依序：① `check_hardware_ready.py --i2c-scan` 確認 `0x68`/`0x69` 都有 ACK；② 讀新增的 `g_wake_result_shoulder`/`g_wake_result_elbow`（`openocd ... mdw`）確認兩顆喚醒都成功；③ 讀 `shoulder_completions`/`elbow_completions` 確認交替邏輯真的有在切換、速率接近預估的 ~290/秒；④ 序列埠/鮑率開啟檢查；⑤ 用 `python3 -m serial.tools.miniterm` 直接看幾行原始輸出，確認格式對，再信任 `run_demo_live.py` 的正規表達式解析（它用 `.search()`、不匹配就靜默 `continue`，格式錯誤會表現成「畫面完全不動」而不是報錯，容易誤判）；⑥ 最後才是搖動兩顆感測器、確認 MuJoCo 對應的自由度動的是**正確那一個**（這是唯一沒辦法靠暫存器讀值驗證的一步，因為兩顆感測器電氣上完全相同，只能靠實際搖哪顆動哪邊來分辨有沒有搞混位址）。

  * **階段 6 實體硬體驗證：長時間除錯 session，找到並修好一連串真實 bug（2026-08-23）**——終於實際接上雙 IMU + MyoWare 跑 `run_demo_live.py`，過程中連續遇到好幾個真實硬體問題，全部靠 SWD/診斷計數器查證根因，不是憑猜測修正。

    - **問題 1：拔掉 IMU#2 電源，IMU#1 也跟著死掉，根因是「斷電但訊號線還接著」**——一開始只拔 `VIN`/`GND`（不拔 `SDA`/`SCL`），該顆 MPU6050 斷電但腳位仍實際接在共用 bus 上；如果斷電當下晶片剛好卡在驅動 `SDA` 為低電位的狀態，斷電後可能維持釘死該電位，因為 I2C 是所有裝置共用同一條線，這會讓整條 bus 對所有裝置都卡死。新增 `ImuReader::nack_count()`/`timeout_count()`（每顆獨立計數）才第一次能分清楚「這顆裝置本身沒回應」（NACK，per-device）跟「整條 I2C1 週邊卡死」（timeout，不分裝置）——先前只有 completions 計數器，兩種情況都表現成 `completions=0`，無法區分。
    - **修法：`i2c1_bus_recovery()`，依 I2C-bus 規格書（NXP UM10204 §3.1.16「Bus clear」）實作的正規 bus-clear 程序**——偵測到卡死時，把 `PB6`/`PB7` 從 I2C1 週邊暫時借回來當一般 GPIO，手動打最多 9 個 `SCL` 時脈脈衝(給卡住的裝置機會在某次時脈邊緣放開 `SDA`)，再補一個 `STOP` 條件，最後交還給 I2C1 週邊、重新 `SWRST`。比原本單純 `SWRST`(只重置 STM32 自己的週邊，救不回外部裝置真的把線釘死的狀態)更徹底。新增 `g_bus_recovery_attempts`/`g_bus_recovery_freed` 計數器佐證機制真的有在動作、且目前實測成功率 100%。
    - **問題 2：I2C completions 正常、但 `shoulder_pitch`/`shoulder_roll`/`elbow` 卻永遠精確卡在 `0.0`**——根因是 MPU6050 斷電重開機後 `PWR_MGMT_1` 的 `SLEEP` 位元恢復預設值 1，I2C 通訊層完全正常(晶片照樣會回應讀寫)，但感測資料暫存器從沒被寫入過、永遠讀到重置預設值 0。開機時的一次性喚醒指令只在 `main()` 最上面跑一次，運行中若晶片因斷電重置又睡著，且是透過「乾淨 NACK 直接停止」這種不會觸發 timeout 的方式恢復連線，就永遠不會被重新喚醒。
    - **修法演進，值得記錄的設計教訓**：第一版做法是「讀到的 `ax=ay=az` 剛好都是 0（現實中不可能，地球重力必有分量）就補發一次喚醒指令」——這是**症狀導向的反應式 patch**，被使用者當場點出問題：不該每次發現新症狀就加一個特判，應該找出真正的規則。重新設計成 `ImuReader::needs_rewake_` 旗標：**任一次 NACK 或 timeout 都會標記該讀取器「需要重新喚醒」，下次它重新成功完成讀取時，一律無條件先補發喚醒指令**（`consume_needs_rewake()`，check-and-clear 語意）——不去猜測資料看起來像不像睡著，而是把「這顆裝置剛從失敗中恢復」本身當成觸發條件，統一涵蓋 NACK 恢復（原本會漏掉的路徑）跟 timeout 恢復（`i2c1_bus_recovery()` 早就有處理）兩種情境，未來任何新的失敗模式也會自動被涵蓋，不用再加特例。
    - **已知但刻意延後的問題：開機時序競爭（power-on race）**——STM32 通電後啟動並嘗試喚醒 I2C 裝置的速度，可能快過剛通電的 MPU6050 本身完成內部穩壓/振盪器啟動的時間，導致冷開機（例如剛換裝感測器模組後）第一次喚醒嘗試就失敗、卡進 `blink_code()` 無窮迴圈。目前的 workaround 是先跑一次 `check_hardware_ready.py --live-check`（它會重新燒錄，燒錄前的等待時間剛好讓感測器穩定，之後再重置就會成功），成因已確認、修法（開機喚醒前加一段保守延遲，例如 100ms）已提出但**刻意延後**，因為目前有可靠的 workaround，優先度較低。

  * **EMG 訊號鏈根因追查：MyoWare 訊號一直卡在固定值、不隨出力變化，根因是 STM32 端接觸不良，不是感測器或韌體（2026-08-23）**——用系統性排除法定位：依序排除電極貼合/位置（重新貼過無效）、MyoWare 供電狀態（拔電源訊號一樣不變）、I2C 雜訊耦合（I2C 確認閒置穩定時問題依舊存在）、韌體 ADC 取樣邏輯（改燒一份完全獨立、早期已驗證過的 `adc_hello_main.c` 仍是同樣症狀，證明不是這次改的程式碼的問題）。**決定性證據**：MyoWare 板子自己內建的出力指示燈能正確、即時對應真實握拳/放鬆節奏（證明感測器本身完全正常），但 STM32 讀到的原始 ADC 值完全沒反應；電表直接量 `ENV→PA0` 那條線本身（不經過 STM32），碰觸會有反應——代表線本身是活的、浮接，但訊號沒有真的傳到 `PA0`。**結論：那條線在 STM32 端的針腳接觸不良**（很可能是當天反覆插拔麵包板造成），換一個新孔位重新插緊後，訊號完全恢復正常（放鬆 ~2000-2150、握拳 ~3600-3700，動態範圍乾淨）。
    - **順帶發現增益已經漂移，重新校準**：新 baseline（放鬆 ~2000-2150）遠高於 2026-08-18 記錄的原始 baseline（~450），代表 MyoWare 板上的增益電位器這段時間被調高了（可能是反覆插拔時不小心碰到）。舊的 `kFallbackThreshold=2037.0f`（依當年低增益 baseline 設定）此時已經落在新 baseline 範圍內，靜止時有機率被誤判成「正在出力」。改成 `kFallbackThreshold=2800.0f`，離新 baseline 高點與握拳低點都留有安全邊界。

  * **診斷工具同步強化，修掉「靠猜測判斷斷線」的假警報（2026-08-23～25）**：
    - **`tools/check_hardware_ready.py --live-check`**：原本判斷 `elbow` IMU 是否正常，是看 `elbow` 這個欄位的數值有沒有變化——但 `elbow` 其實是 `shoulder_pitch - elbow_pitch` 算出來、且被 clamp 到 ≥0 的**衍生值**，手臂擺在某些姿勢會讓這個差值整段時間都是負的、被砍成 0，跟兩顆 IMU 是否真的連線完全無關。已改成讀取韌體新增的 `diag` 那行（`shoulder_completions`/`elbow_completions`/`nacks`/`timeouts`），這是真正的 I2C 協定層級訊號，不是猜的。修好後在真實硬體上重現過兩次「手臂大幅晃動、`elbow` 欄位長時間為 0，但 diag 顯示兩顆完全健康」的情境，新邏輯正確不誤判。
    - **`tools/mujoco_bridge/run_demo_live.py`**：同樣的問題（`frozen_fields()`/滑動視窗判斷數值有沒有變化）整支移除，改用同一套 diag completions 邏輯。新增 `--log-file`（tee 到檔案，因為除錯過程的終端機輸出量已經大到超過對話訊息長度上限）；新增每顆 IMU 離線期間的「重試次數」累計（`imu_retry_attempts()`，恢復連線時歸零）；狀態輸出從單行 `{ IMU#1: ... }` 改成多行 `[SENSORS]` 區塊（避免窄視窗被截斷），維持 edge-triggered + 心跳週期，離線中每次重試次數增加也算變化，確保還在重試的狀態會持續可見。

  * **硬體庫存更新：5 顆 MPU6050 已焊排針，目前測試皆正常（2026-08-29）**——3 顆 GY-521（含階段 4b 記錄過反覆斷線問題的舊模組）+ 2 顆 Adafruit（`0x68`/`0x69` 各一）。額外驗證：把其中一顆 GY-521 換到 shoulder 位置（`0x68`）測試，因為兩者是同一顆 MPU6050 晶片、GY-521 預設位址同樣是 `0x68`，**完全不需要改韌體或位址設定，純物理替換**。長期仍以 Adafruit 板子為主力（已內建 STEMMA QT 免焊接頭、10kΩ 上拉電阻有官方保證，GY-521 則有階段 4b 記錄在案的可靠度前科），GY-521 純粹作為備用/測試用途。
...
逐次開發/除錯 session 的詳細記錄（發現的 bug、排查過程、驗證結果、下個 session 的 TODO）已經搬到 [SESSION_LOG.md](SESSION_LOG.md)，PRD 本身只保留產品規格、架構與路線圖。
